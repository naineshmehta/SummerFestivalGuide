...
dnnModule.digitalAssets = function ($, $find, $telerik, dnnModal) {
    ...
    function refreshPager() {
        ...
        if (totalItems == 0) {
            $(".dnnModuleDigitalAssetsGrid .rgWrap.rgArrPart1").hide();
        ...
        $(".dnnModuleDigitalAssetsGrid .rgWrap.rgArrPart1").show();
    ...
    function internalResetGridComponents(folderId, pattern) {
        var index = grid.getColumnByUniqueName('ParentFolder').get_element().cellIndex;
        grid.hideColumn(index);
        $('#' + controls.gridId + '>table>thead>tr>th:last').show().hide(); // FF workaround to hide the space of the last column
        grid.clearSort();        
        resetSearch(pattern);
    }
    ...
    function setupContextMenu(index, event) {
        ...
        if (!areOnlyFilesSelected(items)) {
            $(menuSelector + " a.rmLink.onlyFiles").each(function () {
                $(this).parent().hide();
            });
        }
    ...
    function createNewFolder(node, sender) {
        ...
        $("#dnnModuleDigitalAssetsCreateFolderModal").dialog({
            modal: true,
            autoOpen: true,
            dialogClass: "dnnFormPopup",
            ...
            height: 400,
            buttons: [
                { id: "save_button", text: resources.saveText, click: function () { saveNewFolder(node, $(this), sender); }, "class": "dnnPrimaryAction" },
    ...
    function initializeCreateNewFolderForm(node) {
        ...
        var folderProviderValue = node.get_category();
    ...
    function saveNewFolder(node, modal, sender) {
    ...
    function getNewUploadFileResultZone(id) {
        return $("<div class='dnnModuleDigitalAssetsUploadFileFile' data-fileoverwrite='false' data-filename='" + id + "'>" +
                ...
                "<div class='dnnModuleDigitalAssetsUploadFileProgressBar ui-progressbar'><div class='ui-progressbar-value'/></div>" +
    ...
    function getUrl() {
        var items = convertToItemsFromGridItems(grid.get_selectedItems());
        var fileId = items[0].ItemId;
        getUrlFromFileId(fileId);
    }
