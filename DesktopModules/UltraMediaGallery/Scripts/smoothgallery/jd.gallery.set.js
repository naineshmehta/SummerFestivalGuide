...
var gallerySet = new Class({
	...
	changeGallery: function(number)
	{
		...
		//Below "if" statement is added by Pengcheng Rong
		if (this.options.showArrows)
		{
			if (this.galleryData.length > 1)
			{
				$(document.body).getElement('.left').setStyle('display','');
				$(document.body).getElement('.right').setStyle('display','');
			}
			else
			{
				$(document.body).getElement('.left').setStyle('display','none');
				$(document.body).getElement('.right').setStyle('display','none');
			}
		}
	},
		...
		this.gallerySet.each(function(galleryItem, index){
			var button = new Element('div').addClass('galleryButton').injectInside(
				...
				'click': function(myself, number){
					//Below line is added by Pengcheng Rong
					beginUMGNotify();
