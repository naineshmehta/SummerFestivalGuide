...
var MooFlow = new Class({
	...
	getElements: function(el){
		...
		$$(els).each(function(el){
			var hash = $H(el.getElement('img').getProperties('src','title','alt','longdesc','id'));
	...
	update: function(e){
		...
		if(this.options.useSlider){	
			...
			var knobWidth = (this.slider.getSize().x/this.iL);
			if (knobWidth < 20)
				knobWidth = 20;
			this.knob.setStyle('width',knobWidth);
	...
	glideTo: function(index){
		...
		if (typeof(UPG_onPhotoLoad) != "undefined") UPG_onPhotoLoad(this.master.images[index].id);
	},
	process: function(x){
