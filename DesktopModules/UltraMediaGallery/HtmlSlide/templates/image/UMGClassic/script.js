	...
	slide[MODULEID].ensureVisibility = function(index, compareToIndex, speed)
	{
		...
		if (index > compareToIndex)
		{
			...
			if (xOffset > this.umg_large.width() * .67)
			{
		...
		else
		{
			...
			if (xOffset < this.umg_large.width() * .33)
			{
				var scrollTo = (currentThumb.width() / 2 + currentThumb.position().left)*-1 + this.umg_container.width() * .5;
				if (scrollTo > 0)
				{
	...
	slide[MODULEID].umg_container.bind("mouseenter", function()
		{
			if (slide[MODULEID].umg_caption.position().top >= 0)
			{
				return;
			}
			slide[MODULEID].umg_caption.animate({top:0},"fast");
		}
	);

	slide[MODULEID].umg_container.bind("mouseleave", function()
		{
			if (slide[MODULEID].umg_caption.position().top <= 0-slide[MODULEID].umg_caption.height())
			{
				return;
			}
