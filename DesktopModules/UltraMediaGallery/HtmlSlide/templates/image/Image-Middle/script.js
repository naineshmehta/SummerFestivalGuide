	slide[MODULEID].buildThumbList = function()
	{
		for (var i=0;i<this.umg_images.length ;i++ )
		{
			...
			{
				item = "<a href='javascript:void(0);' onmouseover='slide[MODULEID].navigateTo("+i+")'  class='umg_page' id='page[MODULEID]_"+i+"'><img border='0' src='"+jQuery(this.umg_images[i]).attr("alt")+"'>"+jQuery(this.umg_titles[i]).html()+"</" + "a>";
			}
			else if (this.umg_mouseoverbehavior == "scroll")
			{
				item = "<a href='javascript:slide[MODULEID].navigateTo("+i+")' onmouseover='slide[MODULEID].hoverTo("+i+")' class='umg_page' id='page[MODULEID]_"+i+"'><img border='0' src='"+jQuery(this.umg_images[i]).attr("alt")+"'>"+jQuery(this.umg_titles[i]).html()+"</" + "a>";
			}
			else //default, do nothing
			{
				item = "<a href='javascript:slide[MODULEID].navigateTo("+i+")' class='umg_page' id='page[MODULEID]_"+i+"'><img border='0' src='"+jQuery(this.umg_images[i]).attr("alt")+"'>"+jQuery(this.umg_titles[i]).html()+"</" + "a>";
			}
	...
	slide[MODULEID].ensureVisibility = function(index, compareToIndex, speed)
	{
		...
		if (index > compareToIndex)
		{
			...
			if (yOffset > this.umg_container.height() * .67)
			{
		...
		else
		{
			...
			if (yOffset < this.umg_container.height() * .33)
			{
				var scrollTo = (currentThumb.height() / 2 + currentThumb.position().top)*-1 + this.umg_container.height() * .5;
				if (scrollTo > 0)
				{
