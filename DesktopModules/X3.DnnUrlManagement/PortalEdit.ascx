<%@ Control Language="C#" AutoEventWireup="true" Explicit="True" Inherits="X3.DnnUrlManagement.PortalEdit"
    CodeBehind="PortalEdit.ascx.cs" %>
...
<div class="DnnUrlManagement dnnForm">
    ...
    <ul class="dnnAdminTabNav">
        ...
        <li><a href="#Pages">Custom Page URLs</a></li>
    </ul>

    <div id="Basic" class="dnnClear">

        <div class="dnnFormItem">
            <dnn:Label ID="lblErrorPage404" runat="server" Text="404 Error Page:" ControlName="ddlErrorPage404" HelpText="The 404 Error Page for your site.  Users will be redirected to this page if the URL they are navigating to results in a 'Page Not Found' error." />
            <dnn:DnnPageDropDownList ID="ddlErrorPage404" runat="server" />
        </div>

        <div class="dnnFormItem">
            <dnn:Label ID="lblErrorPage500" runat="server" Text="500 Error Page:" ControlName="ddlErrorPage500" HelpText="The 500 Error Page for your site.  Users will be redirected to this page if the URL they are navigating to results in an unexpected error." />
            <dnn:DnnPageDropDownList ID="ddlErrorPage500" runat="server" />
        </div>
    ...
    <div id="Pages" class="dnnClear">
        ...
        <div class="dnnFormItem">
            <dnn:Label ID="lblPage" runat="server" Text="Selected Page:" ControlName="ddlPage" HelpText="Select a page from the list to edit the page URLs." />
            <dnn:DnnPageDropDownList ID="ddlPage" runat="server" autopostback="true" OnSelectionChanged="ddlPage_SelectionChanged" />
        </div>


        <div>
            <dnn:Label ID="lblCustomUrls" runat="server" Text="Custom Urls:" ControlName="gvCustomUrls" HelpText="URLs that have been created for the selected page." />
            <div class="dnnClear">
                <telerik:RadGrid ID="gvCustomUrls" runat="server" AutoGenerateColumns="False"
                    OnNeedDataSource="gvCustomUrls_NeedDataSource" OnDeleteCommand="gvCustomUrls_DeleteCommand"
                    OnItemCreated="gvCustomUrls_ItemCreated" OnUpdateCommand="gvCustomUrls_UpdateCommand"
                    OnInsertCommand="gvCustomUrls_InsertCommand">
                    <MasterTableView DataKeyNames="SeqNum" CommandItemDisplay="Top" InsertItemDisplay="Top" EditMode="InPlace">
                        <CommandItemSettings ShowAddNewRecordButton="true" ShowRefreshButton="false" AddNewRecordText="Add new custom URL" />
                        <Columns>
                            <telerik:GridEditCommandColumn ButtonType="ImageButton" UniqueName="Edit" HeaderStyle-Width="1%" ItemStyle-Wrap="false" />
                            <telerik:GridTemplateColumn HeaderText="PortalAlias" UniqueName="PortalAliasId">
                                <ItemTemplate>
                                    <%# GetPortalAlias((int)Eval("PortalAliasId")) %>
                                </ItemTemplate>
                                <EditItemTemplate>
                                    <asp:DropDownList runat="server" ID="ddlPortalAliasId" />
                                </EditItemTemplate>
                            </telerik:GridTemplateColumn>
                            <telerik:GridBoundColumn HeaderText="Url" DataField="Url" />
                            <telerik:GridTemplateColumn HeaderText="URL Type">
                                <ItemTemplate>
                                    <%# GetTabUrlType(Eval("HttpStatus").ToString()) %>
                                </ItemTemplate>
                                <EditItemTemplate>
                                    <asp:DropDownList runat="server" ID="ddlHttpStatus">
                                        <asp:ListItem Text="Active (200)" Value="200" Selected="True" />
                                        <asp:ListItem Text="Redirect (301)" Value="301" />
                                    </asp:DropDownList>
                                </EditItemTemplate>
                            </telerik:GridTemplateColumn>
                            <telerik:GridBoundColumn HeaderText="Query String" DataField="QueryString" />
                            <telerik:GridButtonColumn ConfirmText="Delete this Url?" ConfirmDialogType="RadWindow" ConfirmTitle="Delete" ButtonType="ImageButton" CommandName="Delete" />
                        </Columns>
                        <EditFormSettings>
                            <EditColumn ButtonType="ImageButton" />
                        </EditFormSettings>
                        <NoRecordsTemplate>
                            No custom URLs found
                        </NoRecordsTemplate>
                    </MasterTableView>
                </telerik:RadGrid>
            </div>
        ...
        <div class="dnnFormItem">
            <dnn:Label ID="lblSystemUrl" runat="server" Text="System Generated URL:" ControlName="hypSystemUrl" HelpText="The default URL generated by DNN using all applied settings." />
            <asp:HyperLink runat="server" ID="hypSystemUrl" />
        </div>
